...
public class UpdateConteudoPortalSaude {

    private final ConteudoPortalSaudeGateway gateway;

    public ConteudoPortalSaude execute(final ConteudoPortalSaude conteudo) {
        gateway.findById(conteudo.getId())
                .orElseThrow(() -> new ConteudoNaoEncontradoException(conteudo.getId()));

        return gateway.update(conteudo);
    }
}
